...
\begin{table}[h]
\centering
\caption{Cross-scale beta calibration. $\beta^* = \text{gap\_mean}/0.267$ is the theoretical phase transition — confirmed experimentally at all three scales. At $\beta=1.5$: safely above 135M and 360M transitions (all 4-bit); \emph{below} 1.7B transition (all 8-bit). Setting $\beta \approx \beta^*{+}0.1$ provides a safe margin.}
\begin{tabular}{lccccl}
\toprule
Scale & gap mean & $\beta^*$ (theory) & $\beta^*$ (measured) & $\beta=1.5$ outcome & Source \\
\midrule
135M & 0.330 & 1.233 & $[1.2, 1.3]$ & 100\% 4-bit (above) & CPU experiment \\
360M & 0.337 & 1.260 & $[1.2, 1.4]$ & 100\% 4-bit (above) & CPU experiment \\
1.7B & \textbf{0.424} & \textbf{1.584} & $\mathbf{[1.5, 1.6]}$ & \textbf{0\% 4-bit (below)} & \textbf{CPU experiment} \\
\bottomrule
\end{tabular}
\label{tab:betastar}
\end{table}

The 1.7B experiment (\textbf{new measured result}) reveals $\beta^*=1.584$, placing $\beta=1.5$ just below the transition. At $\beta=1.6$ ($> \beta^*$), the controller achieves 68.4\% 4-bit allocation (2.69$\times$ FPGA speedup), Pareto-dominating DWB (2.44$\times$) at similar avg\_bits (5.26 vs. 5.05).
