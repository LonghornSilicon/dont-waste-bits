...
Concretely: deployments that must match DWB's accuracy (ML-benchmark SLAs, accuracy-first products) should pick $p_4 \in \{0.74, 0.81\}$, trading 0 to 0.3 pp for $+$15--21\% throughput. Deployments that are BRAM-bandwidth-bound (on-device LLMs where the bottleneck is memory bandwidth to compute) can take $p_4 \approx 0.96$, trading 1--2 pp for $\approx+$38\% throughput --- still well away from the static-INT4 accuracy cliff at $p_4=1.0$ ($-$7.5 pp). We explicitly frame this as a design choice: the contribution of this paper is the frontier, not a single winner.
...
0.96$^{\S}$ & \textit{(Phase 7i pending)} & 3.36$\times$ & \textit{pending} / $+38\%$ & BRAM-bound \\
\midrule
