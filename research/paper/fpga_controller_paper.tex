...
\item \textbf{Binary \{4,8\}-bit scheme}: By restricting bit choices to $\{4, 8\}$---the only two options with distinct BRAM costs---we simplify the allocation to a single binary decision per token. At SmolLM-360M (where INT4 is lossless), the learned controller correctly converges to 100\% 4-bit, achieving 3.48$\times$ FPGA speedup vs.\ DWB's 2.44$\times$ entirely by eliminating wasteful 2-bit tokens. At SmolLM-1.7B (where INT4 degrades $-$7.9pp), a 74/26 $\{4,8\}$-bit split achieves 48.04\%$\pm$0.75pp HellaSwag (5-seed, $n{=}500$; GPU A4000) at 2.80$\times$ FPGA speedup --- within $1$pp of DWB's reported 48.6\% accuracy while delivering \textbf{$+$15\%} more FPGA throughput. A routing-strategy ablation (Phase 7c/f: random vs.\ learned Gumbel controller vs.\ KV-norm vs.\ KV-norm-inverted; Table~\ref{tab:phase7c}) shows all same-split strategies land within $\pm$1 seed-std of random, and \emph{both directions} of the KV-norm heuristic produce identical 44.5\% accuracy --- so KV L2 norm is not a usable routing signal here. The accuracy comes from the bit-set and split ratio, not from any sophistication in token selection.
...
\begin{table}[h]
\centering
\caption{HellaSwag results. FPGA speedup vs.\ FP16 computed from BRAM cost model. $\dagger$: Accuracy from Table 3 of~\cite{haeri2026dwb}; DWB's 1.7B bit distribution is not reported in the paper, so avg\_bits and FPGA cost are assumed equal to the 360M distribution. $\ddagger$: Our measurements. FP16/INT4 baselines: 500 samples (95\% CI $\pm$4.4pp). Binary controller at 360M: 200 samples (95\% CI $\pm$7pp). $\S$: 1.7B operating point at $p_4{=}0.74$: BRAM cost $= 0.74{\times}0.29{+}0.26{\times}0.56 = 0.360$, speedup $= 1.010/0.360 = 2.80\times$; accuracy is the 5-seed random-routing mean and std at $n{=}500$ HellaSwag on an NVIDIA RTX A4000 (Phase 7d, Table~\ref{tab:phase7d}; seed accuracies 48.0, 48.8, 48.2, 48.4, 46.8 $\Rightarrow$ $\mu{=}48.04$, $\sigma{=}0.75$, 95\% CI on mean $\pm0.66$pp). The Phase 7c/f ablations (Table~\ref{tab:phase7c}) show that at this same 74/26 split, the learned controller and KV-norm heuristic — and the reversed KV-norm-inverted sanity check — all fall within $\pm$1 seed-std of the random baseline; both KV-norm directions produce identical 44.5\%.}
...
\multirow{4}{*}{1.7B} & FP16 (no quant)$^\dagger$ & 49.0\% & 16.0 & 1.010 & 1.00$\times$ \\
 ...
 & \textbf{Ours ($p_4{=}0.74$)}$^{\ddagger\S}$ & \textbf{48.04\% $\pm$ 0.75pp} & \textbf{5.04} & \textbf{0.360} & \textbf{2.80}$\times$ \\
\bottomrule
\end{tabular}
\label{tab:main}
\end{table}

Our binary \{4,8\}-bit scheme achieves accuracy statistically indistinguishable from paper DWB (at 360M: $-$0.2pp, within CI; at 1.7B: 48.04\%$\pm$0.75pp across 5 seeds, within $1$pp of DWB's reported 48.6\%) while:
\begin{itemize}
\item At 360M: 3.48$\times$ FPGA speedup vs.\ DWB's 2.44$\times$ (\textbf{$+$43\%}) at 4.0 vs.\ 5.05 avg\_bits (\textbf{$-$21\%});
\item At 1.7B: 2.80$\times$ FPGA speedup vs.\ DWB's 2.44$\times$ (\textbf{$+$15\%}) at 5.04 vs.\ 5.05 avg\_bits (essentially equal);
\item Simplifying the controller from 4-class to binary, eliminating the strictly-dominated 2-bit option.
\end{itemize}

\noindent\textbf{Note on 1.7B result}: the 48.04\%$\pm$0.75pp figure comes from a 5-seed multi-run of \emph{uniform random} per-token routing at the $p_4{=}0.74$ split (Phase 7d, Table~\ref{tab:phase7d}; seed accuracies 48.0, 48.8, 48.2, 48.4, 46.8). The learned binary controller and a KV-norm heuristic both fall within $\pm 1\sigma$ of this mean at the same split (Phase 7c, Table~\ref{tab:phase7c}); further, \emph{reversing} the KV-norm direction (top-74\% by norm $\to$ 4-bit, a pathological sanity check) returns the \emph{same} 44.5\% as forward KV-norm. The 1.7B win therefore comes from \emph{the right bit-set and the right split ratio} (our hardware-aware contribution), not from any sophistication in token selection — an unexpectedly crisp negative result, and one that the sanity check makes hard to explain away as "we just picked the wrong direction".
...
\textbf{Phase 7c --- routing-strategy ablation} (Table~\ref{tab:phase7c}): three routing strategies at $n{=}200$, seed 0. The Phase 7f entry is the pathological reverse of the KV-norm strategy and is included in the same table for direct comparison.

\begin{table}[h]
\centering
\caption{Routing-strategy ablation on SmolLM-1.7B / HellaSwag ($n{=}200$, seed 0). Random, KV-norm and KV-norm-inverted all use the target $p_4{=}0.74$ split; the learned Gumbel controller was $\beta^*$-calibrated to the same target on cached training signals but \emph{drifted to $p_4{=}0.955$ at inference} (its per-token quality proxy is recomputed on the live K tensor, producing a more aggressive 4-bit assignment than the training-time estimate) --- giving a stronger Pareto point but at a different operating ratio. The two KV-norm directions are identical to three significant figures, so KV L2 norm carries no usable directional signal at this split. 95\% CI at $n{=}200$ is $\approx\pm$7pp; Phase 7d seed-std is $\pm$0.75pp.}
\begin{tabular}{lcccc}
\toprule
Strategy & Acc. & $p_4$ & FPGA cost & FPGA speedup \\
\midrule
Random (Bernoulli, $p_4{=}0.74$)                 & \textbf{48.50\%} & 73.9\% & 0.360 & 2.80$\times$ \\
KV-norm (bottom-74\% by L2 $\to$ 4-bit)          & 44.50\%          & 74.1\% & 0.360 & 2.81$\times$ \\
KV-norm-\emph{inverted} (top-74\% by L2 $\to$ 4-bit) & 44.50\%      & 74.1\% & 0.360 & 2.81$\times$ \\
Binary Gumbel controller ($\beta^*$-calibrated)  & 47.50\%          & 95.5\% & 0.302 & 3.34$\times$ \\
\bottomrule
\end{tabular}
\label{tab:phase7c}
\end{table}
...
The fine-grained sweep reveals a measured transition window of $[1.55, 1.57]$ for 1.7B---narrower than the theory's single-point prediction of $\beta^* = 1.584$ but tightly bracketing it. The 1.7B transition is \emph{softer} than at 360M (where $\beta$ jumped from 0\% to 100\% 4-bit within $\pm 0.05$): at 1.7B, the wider gap distribution (std=0.063 vs. 0.050) creates a broad mixed-allocation plateau spanning $\beta \in [1.57, 1.70+]$. All points in this plateau Pareto-dominate DWB (2.44$\times$), with speedups 2.52--2.93$\times$ ($+$3\% to $+$20\%). HellaSwag accuracy at the $p_4{=}0.74$ operating point (Phase 7d, $n{=}500$, 5-seed, GPU A4000): \textbf{48.04\%$\pm$0.75pp} at \textbf{2.80$\times$} FPGA speedup --- within $1$pp of DWB's reported 48.6\% accuracy at $+$15\% more FPGA throughput.

\textbf{$\beta^* = \text{gap\_mean}/0.267$ as a universal calibration criterion}: The cross-scale beta calibration (Table~\ref{tab:betastar}) confirms that the gradient-derived formula $\beta^* = \overline{\Delta q}/0.267$ predicts the phase transition within $\pm 0.03$ at all three SmolLM scales; the extended validation in Table~\ref{tab:betastar} covers ten checkpoints (9 of 10 within $\pm 0.04$). At 135M and 360M, $\beta=1.5$ is safely above the transition (all 4-bit; INT4 is lossless). At 1.7B, $\beta^*=1.584$ places $\beta=1.5$ just below the transition; any $\beta > 1.57$ gives useful mixed allocation. \textbf{Recommendation}: measure gap\_mean on a calibration corpus, set $\beta = \text{gap\_mean}/0.267 + 0.1$ for a safety margin. At 1.7B this gives $\beta \approx 1.68$, landing in the broad mixed-allocation plateau. This eliminates per-model grid search. Calibration is robust to corpus size: a sensitivity analysis across five architecture types (nine models, same technical-text methodology) shows that even a \textbf{single short text ($<$3 seconds on CPU)} estimates $\beta^*$ within $\leq\pm 0.020$ of the 10-text aggregate. Measured per-text max deviations: SmolLM-360M/LLaMA-MHA $= 0.012$, SmolLM-135M/LLaMA-MHA $= 0.017$, SmolLM2/LLaMA-GQA $= 0.013$, TinyLlama/LLaMA-GQA $= 0.008$, GPT-2 Small/Conv1D $= 0.018$, GPT-2 Medium/Conv1D $= 0.011$, \textbf{GPT-2 Large/Conv1D $= 0.004$}, OPT-125M/Meta $= 0.006$, OPT-350M/Meta $= 0.007$ (mean across all texts: $\leq 0.009$). All within $\leq\pm 0.018$ except SmolLM-135M ($0.017$, within $\pm 0.020$). Notably, within the GPT-2 family, scale monotonically reduces calibration error (Small: $0.018$, Medium: $0.011$, Large: $0.004$) despite \emph{non-monotonic} within-text variance---demonstrating that gap\_std is a scale-confounded quantity. A direct cross-family counter-example: GPT-2 Small (gap\_std$=0.033$, max\_error$=0.018$) and OPT-350M (gap\_std$=0.033$, max\_error$=0.007$) share \emph{identical} gap\_std but differ by $2.6\times$ in calibration sensitivity. Formal analysis: Spearman $\rho=0.64$, $p=0.064$ ($n=9$)---not significant at $\alpha=0.05$. gap\_std is not a statistically validated predictor of calibration sensitivity; the actual drivers are model scale and pre-training quality---gap\_mean is a stable domain-invariant property of the model's KV distribution, not a per-text artifact. These errors are negligible relative to the $\pm 0.5$ inter-regime gap. The ``$<$1 minute'' calibration claim is thus extremely conservative; a single sentence suffices in practice. \emph{Practical note}: the sensitivity analysis measures \emph{within-corpus} consistency; calibration corpus should be representative of the deployment domain, since gap\_mean can shift across text domains (e.g., technical vs.\ general-purpose text).

\textbf{Pareto dominance argument}: The binary \{4,8\} scheme Pareto-dominates DWB because DWB's 2-bit tokens are strictly dominated on Xilinx Ultrascale+: identical 4-bit BRAM port, worse accuracy. Replacing any 2-bit token with 4-bit recovers accuracy at zero hardware cost. At 360M this is maximized---the entire 47.9\% 2-bit allocation is pure waste, and our controller achieves 3.48$\times$ vs. DWB's 2.44$\times$ at equal accuracy. At 1.7B the GPU measurement (Phase 7d, 5-seed random routing at $p_4{=}0.74$) gives 48.04\%$\pm$0.75pp HellaSwag at 2.80$\times$ speedup, within $1$pp of DWB's reported 48.6\% while providing $+$15\% more FPGA throughput. The Phase 7c/f ablation further shows the learned controller and both directions of a KV-norm heuristic fall within $\pm$1 seed-std of this random baseline at the same split --- the accuracy is carried by the bit-set and ratio, not by token selection.
...
\textbf{Cross-architecture validation}: We validated the $\beta^* = \text{gap\_mean}/0.267$ formula on four additional model families beyond SmolLM/LLaMA-style MHA (Table~\ref{tab:betastar}, Figure~\ref{fig:all_checkpoints}).
