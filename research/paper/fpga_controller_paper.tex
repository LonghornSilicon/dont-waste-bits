...
We measured within-corpus calibration sensitivity (how closely a single text estimates $\beta^*$ relative to a 10-text aggregate) across eight models and four architecture types. All experiments use the same 10 technical texts; results are formally archived as JSON files in the repository.

\begin{table}[h]
\centering
\caption{Calibration corpus sensitivity across eight models. \textbf{max\_error}: largest $|\beta^*_\text{1-text} - \beta^*_\text{10-text}|$ across 10 texts. \textbf{mean\_error}: mean absolute error. All within $\leq\pm0.018$ (well inside the $\pm0.5$ inter-regime gap). Spearman $\rho=0.70$, $p=0.054$ ($n=8$, n.s.): gap\_std is not a statistically validated predictor of calibration sensitivity (\textbf{Finding A8}).}
\begin{tabular}{llcccc}
\toprule
Architecture & Model & gap\_std & max\_error & mean\_error & JSON file \\
\midrule
...
Conv1D & GPT-2 Medium (345M) & 0.052 & 0.011 & 0.005 & \texttt{gpt2\_medium\_cal\_sensitivity} \\
Conv1D & GPT-2 Large (774M) & 0.026 & \textbf{0.004} & \textbf{0.003} & \texttt{gpt2\_large\_cal\_sensitivity} \\
Conv1D & GPT-2 Small (124M) & 0.033 & 0.018 & 0.009 & \texttt{gpt2\_cal\_sensitivity} \\
OPT/Meta & OPT-125M & 0.028 & \textbf{0.006} & \textbf{0.002} & \texttt{opt125m\_cal\_sensitivity} \\
OPT/Meta & OPT-350M & 0.033 & 0.007 & 0.003 & \texttt{opt350m\_cal\_sensitivity} \\
\bottomrule
\end{tabular}
\label{tab:sensitivity_full}
\end{table}

Four comparisons collectively invalidate gap\_std as a calibration sensitivity predictor (\textbf{Finding A8}): (1) GPT-2 family (Small$\to$Medium$\to$Large): max\_error \emph{monotonically} improves ($0.018\to0.011\to0.004$) while gap\_std is \emph{non-monotonic} ($0.033\to0.052\to0.026$); (2) direct counter-example: GPT-2 Small and OPT-350M share \emph{identical} gap\_std ($=0.033$) but differ by $2.6\times$ in max\_error ($0.018$ vs.\ $0.007$); (3) both LLaMA-GQA models share identical mean\_error ($0.004$) despite very different formula-fit quality; (4) OPT within-family is flat across $125$M$\to350$M (max\_error $0.006\to0.007$) while GPT-2 shows strong scale-dependent improvement---scale's effect on sensitivity is architecture-dependent. Formal test: Spearman $\rho=0.70$, $p=0.054$ ($n=8$, n.s.).

\textbf{Corpus dependency} (\textbf{Finding A9}): gap\_mean can shift across text domains. OPT-125M: technical texts give gap\_mean$=0.170$ vs.\ wikitext-2 calibration value of $0.213$ ($\Delta=0.043$). GPT-2 Small: $0.233$ vs.\ $0.196$ ($\Delta=0.037$). The sensitivity analysis measures \emph{within-corpus} consistency; users should calibrate on text representative of their deployment domain.
