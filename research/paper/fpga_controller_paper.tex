...
\begin{table}[h]
\centering
\caption{Cross-scale and cross-architecture beta calibration. $\beta^* = \text{gap\_mean}/0.267$ is the theoretical phase transition, confirmed experimentally at four model checkpoints across two architectures. Setting $\beta \approx \beta^*{+}0.1$ provides a safe margin.}
\begin{tabular}{llccccl}
\toprule
Model family & Scale & gap mean & $\beta^*$ (theory) & $\beta^*$ (measured) & $\beta=1.5$ outcome & Source \\
\midrule
\multirow{3}{*}{SmolLM (LLaMA)} & 135M & 0.330 & 1.233 & $[1.2, 1.3]$ & 100\% 4-bit (above) & CPU \\
 & 360M & 0.337 & 1.260 & $[1.2, 1.4]$ & 100\% 4-bit (above) & CPU \\
 & 1.7B & 0.424 & 1.584 & $[1.5, 1.6]$ & 0\% 4-bit (below) & CPU \\
\midrule
OPT (Meta) & 125M & \textbf{0.213} & \textbf{0.798} & $\mathbf{[0.75, 0.80]}$ & \textbf{100\% 4-bit (above)} & \textbf{CPU} \\
\bottomrule
\end{tabular}
\label{tab:betastar}
\end{table}
...
\caption{Beta calibration analysis. Left: quality gap distribution $q_{t,8}^\text{local} - q_{t,4}^\text{local}$ at 135M, 360M (measured) and 1.7B (estimated). The $\beta{=}1.5$ threshold (0.401) correctly separates the 4-bit zone from the 8-bit zone. Right: 4-bit token fraction vs.\ $\beta$ at 135M (measured), 360M (measured) and predicted 1.7B, with theoretical curves overlaid. At 1.7B, $\beta = 1.5$ lands near the 54.1\% minimum needed to exceed DWB's FPGA speedup.}
\label{fig:beta}
\end{figure}
...
1.60 & 62.2\% & 37.8\% & 5.51 & 2.58$\times$ & Mixed \\
...
\textbf{Cross-architecture validation}: We validated the $\beta^* = \text{gap\_mean}/0.267$ formula on OPT-125M~\citep{zhang2022opt} (Meta; OPT architecture vs. SmolLM's LLaMA-style). OPT-125M has significantly lower KV variance than SmolLM (gap\_mean$=0.213$ vs.\ $0.337$--$0.424$), giving $\beta^*=0.798$. A fine-grained sweep confirms the transition at $[0.75, 0.80]$, bracketing the prediction within $\pm 0.023$. At $\beta=0.80$ (threshold $0.214 \approx \text{gap\_mean}$), the controller achieves $48.2\%$ 4-bit---genuine mixed allocation. At any $\beta \geq 1.0$, OPT-125M converges to $100\%$ 4-bit ($3.48\times$, INT4 lossless at this scale). This confirms the formula generalizes across both scale (135M--1.7B) \emph{and} model family (LLaMA-style, OPT-style), validating it as a hardware-derived universal criterion rather than a SmolLM-specific artifact.
