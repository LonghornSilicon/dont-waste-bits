...
We measured within-corpus calibration sensitivity (how closely a single text estimates $\beta^*$ relative to a 10-text aggregate) across seven models and four architecture types. All experiments use the same 10 technical texts; results are formally archived as JSON files in the repository.

\begin{table}[h]
\centering
\caption{Calibration corpus sensitivity across seven models. \textbf{max\_error}: largest $|\beta^*_\text{1-text} - \beta^*_\text{10-text}|$ across 10 texts. \textbf{mean\_error}: mean absolute error. All within $\leq\pm0.018$ (well inside the $\pm0.5$ inter-regime gap). The orthogonality of gap\_std and calibration sensitivity is consistent across all seven: models with high within-text token variance (gap\_std) do \emph{not} show high calibration instability (\textbf{Finding A8}).}
\begin{tabular}{llcccc}
\toprule
...
Three within-family or cross-model comparisons strengthen Finding A8: (1) GPT-2 family (Small$\to$Medium$\to$Large) shows max\_error \emph{monotonically} improves ($0.018\to0.011\to0.004$) while gap\_std is \emph{non-monotonic} ($0.033\to0.052\to0.026$)---definitively ruling out gap\_std as the driver; scale itself reduces calibration error; (2) both LLaMA-GQA models (SmolLM2-360M and TinyLlama-1.1B) share identical mean\_error (0.004) despite differing by 0.75B parameters and formula-fit quality (errors 0.044 vs.\ 0.003); (3) GPT-2 Large achieves the best single-text calibration (max\_error$=0.004$) of all seven models, despite not being the largest model tested.
