...
\textbf{Cross-architecture validation}: We validated the $\beta^* = \text{gap\_mean}/0.267$ formula on OPT-125M~\citep{zhang2022opt} (Meta; OPT architecture vs. SmolLM's LLaMA-style). OPT-125M has lower average INT4 relative error than SmolLM-360M ($\epsilon_\text{rel}=0.228$ vs. $0.270$), which directly drives a lower quality gap (gap\_mean$=0.213$ vs.\ $0.337$--$0.424$) and an earlier phase transition ($\beta^*=0.798$). A fine-grained sweep confirms the transition at $[0.75, 0.80]$, bracketing the prediction within $\pm 0.023$. At $\beta=0.80$ (threshold $0.214 \approx \text{gap\_mean}$), the controller achieves $48.2\%$ 4-bit---genuine mixed allocation at the exact predicted transition. At any $\beta \geq 1.0$, OPT-125M converges to $100\%$ 4-bit ($3.48\times$), consistent with INT4 being lossless at this scale ($\epsilon_\text{eff} \approx \epsilon_\text{rel} \times r_\text{survive} \approx 0.228 \times 0.30 \approx 6.8\% < 8.1\%$ threshold). This confirms the formula generalizes across both scale (135M--1.7B) \emph{and} model family (LLaMA-style, OPT-style): the formula is a hardware-derived criterion that adapts to any model's KV statistics via a single CPU calibration pass.
