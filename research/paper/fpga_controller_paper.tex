...
\textbf{Fine-tuning shifts the KV distribution}: Instruction fine-tuning substantially changes gap\_mean. SmolLM-360M-Instruct (same architecture as the 360M base, but SFT-trained) shows gap\_mean$=0.194$ vs.\ the base model's $0.337$---a 43\% reduction, shifting $\beta^*$ from $1.261$ to $0.727$. At any $\beta \geq 1.0$, the instruct model is in the 100\% 4-bit regime, consistent with lossless INT4 at this scale. The shift is comparable in magnitude to switching architectures (GPT-2 has gap\_mean$=0.196$ at the same $d_\text{model}=768$). This finding has an important practical implication: \textbf{calibration must be run on the exact deployed model checkpoint}, not derived from the base model. Since calibration requires only 1--10 texts ($<3$ seconds), this is a trivially cheap operation.
