...
\midrule
OPT (Meta) & 125M & 0.213 & 0.798 & $[0.75, 0.80]$ & CPU \\
\midrule
...
\emph{OPT-125M}~\citep{zhang2022opt} (Meta; decoder-only, separate k/v projections): gap\_mean$=0.213$, $\beta^*=0.798$. Fine sweep confirms transition at $[0.75, 0.80]$, within $\pm 0.023$ of prediction. At $\beta=0.80$ (threshold $0.214 \approx$ gap\_mean), controller achieves $48.2\%$ 4-bit---genuine mixed allocation at the exact transition.
...
Across all eight checkpoints (scales 124M--1.7B, four model families: SmolLM/LLaMA-MHA, TinyLlama/LLaMA-GQA, OPT/Meta, GPT/OpenAI), the formula error is $\leq\pm 0.04$. The complete GPT-2 family (Small, Medium, Large: three sizes) further reveals a \emph{floor gap\_mean} $\approx 0.19$: all three cluster within $0.008$ regardless of scale or dimension, placing $\beta^* \approx 0.70$--$0.73$. The formula is hardware-derived ($0.267 = (c_8{-}c_4)/C_\text{FP16}$) and architecture-independent: it adapts to any model's KV statistics via a single CPU calibration pass on 1--10 texts ($<3$ seconds).
...
\begin{table}[h]
\centering
\caption{Base vs.\ instruction-tuned gap\_mean and $\beta^*$ for MHA and GQA architectures. SFT reduces gap\_mean by $\approx$44\% in MHA models (SmolLM). GQA models (TinyLlama) show \emph{no measurable shift} ($\Delta=-0.3\%$) because GQA already places gap\_mean at the ``instruction-level floor'' ($\approx$0.18--0.19) by architecture. This floor coincides with SFT-instruct values, GPT-2 Small (0.196), and OPT-125M (0.213)---suggesting $\approx$0.19 is a natural lower bound for well-trained models. Transition window for 360M-Instruct verified by sweep: predicted $\beta^*=0.727$, measured $[0.70, 0.80]$, error$=0.023$.}
