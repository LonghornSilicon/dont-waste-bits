...
\textbf{$\beta = 1.5$ as a near-universal operating point}: A key observation from the fine-grained sweep is that $\beta = 1.5$ is near-optimal across both model scales---for very different reasons. At 360M, $\beta^* = 0.337/0.267 = 1.260$, so $\beta = 1.5$ is safely above the transition threshold, producing 100\% 4-bit (maximum FPGA throughput for a lossless scale). At 1.7B, $\beta^* = 0.400/0.267 = 1.498 \approx 1.5$, placing $\beta = 1.5$ right at the distribution median and producing genuine mixed \{4,8\}-bit allocation (approximately 53.5\% 4-bit). A single hyperparameter value thus \emph{automatically adapts} from pure-4-bit to mixed-precision as the model scale changes---no per-scale tuning is required.
