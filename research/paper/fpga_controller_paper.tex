...
\begin{table}[h]
\centering
\caption{Cross-scale and cross-architecture $\beta^*$ calibration across 6 checkpoints and 4 model families. $\beta^* = \text{gap\_mean}/0.267$ is the theoretical phase transition, confirmed within $\leq\pm0.04$ at every checkpoint. Setting $\beta \approx \beta^*{+}0.1$ provides a safe margin. The formula is hardware-derived (0.267 $=$ $(c_8{-}c_4)/C_\text{FP16}$) and adapts to any model's KV statistics via a single CPU calibration pass. GQA models (TinyLlama) have substantially lower gap\_mean than MHA models at comparable scale---reflecting K/V head dimensionality reduction.}
...
\midrule
GPT-2 (OpenAI) & 124M & 0.196 & 0.733 & $[0.70, 0.80]$ & CPU \\
\bottomrule
\end{tabular}
\label{tab:betastar}
\end{table}
...
Across all six checkpoints (scales 124M--1.7B, four model families: SmolLM/LLaMA-MHA, TinyLlama/LLaMA-GQA, OPT/Meta, GPT/OpenAI), the formula error is $\leq\pm 0.04$. The formula is hardware-derived ($0.267 = (c_8{-}c_4)/C_\text{FP16}$) and architecture-independent: it adapts to any model's KV statistics via a single CPU calibration pass on 1--10 texts ($<3$ seconds).

\textbf{Fine-tuning shifts the KV distribution}: Instruction fine-tuning substantially changes gap\_mean, consistently across scales. SmolLM-135M-Instruct and SmolLM-360M-Instruct (same architectures as their base models, SFT-trained) both show $\approx$44\% lower gap\_mean than their base counterparts (Table~\ref{tab:instruct}):

\begin{table}[h]
\centering
\caption{Base vs.\ instruction-tuned gap\_mean and $\beta^*$. SFT reduces gap\_mean by $\approx$44\% at both scales. Transition window for 360M-Instruct verified by empirical controller sweep (3 seeds, 5 betas): predicted $\beta^*=0.727$, measured $[0.70, 0.80]$, error$=0.023$---within the $\pm 0.04$ bound established for base models.}
\begin{tabular}{llccccc}
\toprule
Model & Variant & gap mean & $\beta^*$ (theory) & Measured transition & $\Delta$gap & $\Delta\beta^*$ \\
\midrule
\multirow{2}{*}{SmolLM-135M} & Base & 0.330 & 1.233 & $[1.2, 1.3]$ & — & — \\
 & Instruct (SFT) & \textbf{0.181} & \textbf{0.677} & — & $-45\%$ & $-0.556$ \\
\midrule
\multirow{2}{*}{SmolLM-360M} & Base & 0.337 & 1.261 & $[1.2, 1.4]$ & — & — \\
 & Instruct (SFT) & \textbf{0.194} & \textbf{0.727} & $\mathbf{[0.70, 0.80]}$ & $-43\%$ & $-0.534$ \\
\bottomrule
\end{tabular}
\label{tab:instruct}
\end{table}

The shift ($-44\%$) exceeds any cross-architecture gap measured (max: $0.040$ between LLaMA and OPT). Instruct models show gap\_mean values ($0.181$--$0.194$) similar to GPT-2 Small ($0.196$, trained on structured WebText), consistent with instruction data regularizing KV activations toward more uniform magnitudes. For SmolLM-360M-Instruct, we verified the predicted transition empirically with a controller sweep: at $\beta=0.70$ (threshold $0.187 < 0.194$) the controller is 15.6\% 4-bit (entering mixed), at $\beta=0.75$ it is 52.6\% 4-bit, and at $\beta \geq 0.80$ it is 100\% 4-bit---bracketing the predicted $\beta^*=0.727$ with error $0.023$, consistent with the $\leq\pm 0.04$ bound. At $\beta \geq 1.0$, both instruct models converge to 100\% 4-bit, consistent with INT4 being lossless at these scales. \textbf{Practical implication}: calibration must be run on the exact deployed model checkpoint---not derived from the base. Since calibration requires only 1--10 texts ($<3$ seconds), this is trivially cheap.
