...
\begin{table}[h]
\centering
\caption{Base vs.\ instruction-tuned gap\_mean and $\beta^*$. SFT reduces gap\_mean by $\approx$44\% at both scales, shifting $\beta^*$ to the $[0.68, 0.73]$ range.}
\begin{tabular}{llcccc}
\toprule
Model & Variant & gap mean & $\beta^*$ & $\Delta$gap & $\Delta\beta^*$ \\
\midrule
\multirow{2}{*}{SmolLM-135M} & Base & 0.330 & 1.233 & — & — \\
 & Instruct (SFT) & \textbf{0.181} & \textbf{0.677} & $-45\%$ & $-0.556$ \\
\midrule
\multirow{2}{*}{SmolLM-360M} & Base & 0.337 & 1.261 & — & — \\
 & Instruct (SFT) & \textbf{0.194} & \textbf{0.727} & $-43\%$ & $-0.534$ \\
\bottomrule
\end{tabular}
\label{tab:instruct}
\end{table}

The shift ($-44\%$) exceeds any cross-architecture gap measured (max: $0.040$ between LLaMA and OPT). Instruct models show gap\_mean values ($0.181$--$0.194$) similar to GPT-2 Small ($0.196$, trained on structured WebText), consistent with instruction data regularizing KV activations toward more uniform magnitudes. At any $\beta \geq 1.0$ (well above $\beta^*=0.727$), both instruct models converge to 100\% 4-bit, consistent with INT4 being lossless at these scales. \textbf{Practical implication}: calibration must be run on the exact deployed model checkpoint---not derived from the base. Since calibration requires only 1--10 texts ($<3$ seconds), this is trivially cheap.
