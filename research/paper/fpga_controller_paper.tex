...
At 1.7B, the gap distribution shifts higher (larger KV errors $\Rightarrow$ lower $q_{t,4}^\text{local}$), placing $\beta = 1.5$ near the distribution median---producing the predicted 40--60\% 4-bit mixed allocation. We extend the controller to 4D features $\mathbf{f}_t = [kv\_norm_t, \text{pos\_frac}_t, q_{t,4}^\text{local}, q_{t,8}^\text{local}]$ and sweep $\beta \in \{1.0, 1.5, 2.0, 3.0\}$, selecting the allocation that maximizes FPGA speedup. Results are pending GPU evaluation.
