...
\midrule
\multirow{2}{*}{GPT-2 (OpenAI)} & 124M & 0.196 & 0.733 & $[0.70, 0.80]$ & CPU \\
 & \textbf{345M} & \textbf{0.188} & \textbf{0.704} & $\mathbf{[0.68, 0.70]}$ & \textbf{CPU} \\
\bottomrule
\end{tabular}
\label{tab:betastar}
\end{table}
...
\emph{GPT-2 Medium}~\citep{radford2019language} (OpenAI, 345M; same Conv1D architecture, $d_\text{model}=1024$, 24 layers, 16 heads): gap\_mean$=0.188$, $\beta^*=0.704$. Fine sweep confirms transition at $[0.68, 0.70]$, within $\pm 0.014$ of prediction. \textbf{Crucially, GPT-2 Medium has \emph{lower} gap\_mean than GPT-2 Small ($0.188$ vs.\ $0.196$) despite its larger hidden dimension ($d=1024$ vs.\ $768$).} This challenges a naive hypothesis that larger K/V dimensions cause higher gap\_mean: more expressive models may learn more \emph{regular} KV representations, converging toward the floor gap\_mean $\approx 0.18$--$0.19$ regardless of raw K/V dimension. GPT-2 Medium clusters with GQA models and MHA-instruct models at this floor, suggesting that \textbf{representation quality---not dimensionality---is the dominant factor}.

Across all seven checkpoints (scales 124M--1.7B, four model families: SmolLM/LLaMA-MHA, TinyLlama/LLaMA-GQA, OPT/Meta, GPT/OpenAI), the formula error is $\leq\pm 0.04$. The GPT-2 family further reveals a \emph{floor gap\_mean} $\approx 0.18$--$0.19$: better-trained, more expressive models cluster here regardless of architecture or dimension, placing $\beta^* \approx 0.70$--$0.73$. The formula is hardware-derived ($0.267 = (c_8{-}c_4)/C_\text{FP16}$) and architecture-independent: it adapts to any model's KV statistics via a single CPU calibration pass on 1--10 texts ($<3$ seconds).
