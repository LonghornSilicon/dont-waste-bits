...
\textbf{Corpus dependency} (\textbf{Finding A9}): gap\_mean shifts across text domains; the magnitude depends on the model regime. OPT-125M: technical texts$\to$gap\_mean$=0.170$ vs.\ wikitext-2$=0.213$ ($\Delta=0.043$). GPT-2 Small: $0.233$ vs.\ $0.196$ ($\Delta=0.037$). Above-floor SmolLM MHA base models show larger shifts: $0.265$ vs.\ $\sim0.337$ ($\Delta\approx0.072$). \textbf{Instruct-tuning effect is corpus-dependent}: SmolLM-360M-Instruct gives gap\_mean$=0.265$ on technical ML texts---\emph{identical} to the base model ($0.265$, $\Delta=0.0002$)---despite the instruct model being at the floor ($0.189$) on general/wikitext corpora. Both base and instruct converge to gap\_mean$\approx0.265$ on technical ML texts, from very different general-text starting points ($0.337$ and $0.189$, respectively). Calibration corpus must be representative of the deployment domain: on technical-text deployments, base and instruct models calibrate identically; on general-text deployments, instruct models require a lower $\beta^*$.
