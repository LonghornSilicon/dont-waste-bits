...
\textbf{Per-token quality proxy}: To enable genuine mixed allocation, we replace global quality scores with a per-token proxy:
\[
q_{t,b}^\text{local} = 1 - \frac{\|\text{Quant}_b(kv_t) - kv_t\|_2}{\|kv_t\|_2}
\]
This gives token-specific quality scores: high-$\|kv\|$ tokens receive lower $q_{t,4}^\text{local}$ (pushed toward 8-bit) while low-$\|kv\|$ tokens stay 4-bit. The per-token quality is available at signal-extraction time (Stage 1) with no additional forward pass. We extend the controller to 4D features $\mathbf{f}_t = [kv\_norm_t, pos\_frac_t, q_{t,4}^\text{local}, q_{t,8}^\text{local}]$.

We predict this produces 60--80\% 4-bit, 20--40\% 8-bit allocation, accuracy $\approx 45$--48\% on HellaSwag, and FPGA cost $\approx 0.35$--0.42 at 1.7B---representing a genuine improvement over DWB's 0.414 FPGA cost while recovering accuracy. Results are pending GPU evaluation.
