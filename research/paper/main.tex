...
($\dagger$) beta=1.0 training-only (val\_acc 0.257 $\approx$ random 0.25);
betas 1.0/2.0 full eval blocked by RAM on test machine (2.5~GB free).}
\label{tab:beta}
\footnotesize
\begin{tabular}{ccccc}
\toprule
...
1.0 & $\approx$N/A$^\dagger$ & $<$3.5 & 0.257 & --- \\
2.0 & \textit{(not run)} & --- & --- & --- \\
\midrule
...
At $\beta=0.5$, avg\_bits drops to 3.92 (below target) while
accuracy is unchanged at 39.0\%---higher latency penalty trades bits
for no accuracy benefit, worsening the compression/accuracy ratio.
At $\beta=1.0$, training val\_acc = 0.257 $\approx$ random (0.25)---the
latency penalty overwhelms the classification loss entirely, leaving the
controller in essentially a random state.
There is no $\beta$ that simultaneously achieves the paper's 41.2\%
accuracy at 5.05 avg\_bits; the compound-loss training procedure used
by the original paper is qualitatively different from our
quartile-classification approach.
