...
compression. Higher $\beta$ pushes bits below target but collapses
accuracy. No $\beta$ achieves the paper's 41.2\% at 5.05 bits.
($\dagger$) 50 eval samples; all others 100 samples.}
\label{tab:beta}
\footnotesize
\begin{tabular}{ccccc}
\toprule
...
At $\beta=0.5$, avg\_bits collapses to 3.06 (far below target) while
accuracy also falls to 37.0\%.
There is no $\beta$ that simultaneously achieves both accuracy and
compression targets; the pattern confirms that compound-loss training
is qualitatively different from our quartile-classification approach.
Betas 1.0 and 2.0 are expected to follow the same monotone trend
(lower bits, lower accuracy).
