...
\begin{table}[h]
\centering
\caption{$\beta$ sweep results: latency penalty vs.\ accuracy/compression
trade-off. Results pending (experiment running).}
\label{tab:beta}
\footnotesize
\begin{tabular}{ccccc}
\toprule
$\beta$ & Acc & avg\_bits & val\_acc & Notes \\
\midrule
0.1 & \multicolumn{4}{c}{\textit{(results pending)}} \\
0.5 & & & & \\
1.0 & & & & \\
2.0 & & & & \\
\midrule
\textit{Paper} & \textit{41.2\%} & \textit{5.05} & --- & target \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Prediction}: higher $\beta$ will lower avg\_bits (compression
improves) but accuracy will also fall, confirming the dual-objective
tension is a fundamental limitation of the quartile-labeling approach—not
merely a hyperparameter choice.
If $\beta=1.0$ or $\beta=2.0$ achieves $\approx$37\% accuracy at
$\approx$5.05 avg\_bits, the gap would be attributable to
$\beta$ alone.
