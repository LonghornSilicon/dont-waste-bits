"""
Calibration sensitivity for SmolLM-135M and SmolLM-360M (LLaMA-MHA, HuggingFaceTB).
SmolLM-360M is the paper's PRIMARY model for all Table 3 claims — yet no formal
sensitivity JSON existed. SmolLM-135M adds the MHA within-family triple (135/360/1700M).
Prediction: above-floor models (gap_mean~0.33) may be MORE sensitive than floor models
(gap_mean~0.18-0.19) because their gap_mean sits in a higher-variance regime.
"""
...
def run_sensitivity(model_id, out_filename):
    print(f"\n{'='*60}")
    ...
    def extract_gaps(text_list):
        ...
        def make_hook(i, proj):
            def h(module, inp, out):
                kv_store[(proj, i)] = out.detach().float()
            return h

        for i in range(n_layers):
            hooks.append(model.model.layers[i].self_attn.k_proj.register_forward_hook(make_hook(i, 'k')))
            hooks.append(model.model.layers[i].self_attn.v_proj.register_forward_hook(make_hook(i, 'v')))
    ...
    print(f"\n{model_id} sensitivity: max_error={max_e:.3f}, mean_error={mean_e:.3f}")
    print(f"  Within +-0.015: {max_e <= 0.015}  Within +-0.020: {max_e <= 0.020}")

    n_layers_str = f"{n_layers}L"
    result = {"model": model_id, "architecture": f"LLaMA-MHA/SmolLM ({n_layers_str})",
              "note": "SmolLM base (above-floor gap_mean~0.33). Primary paper model (360M) or within-family comparison.",
    ...
    with open(f"{RESULTS_DIR}/{out_filename}", "w") as f:
...
r135 = run_sensitivity("HuggingFaceTB/SmolLM-135M", "smollm_135m_cal_sensitivity.json")
r360 = run_sensitivity("HuggingFaceTB/SmolLM-360M", "smollm_360m_cal_sensitivity.json")

print("\n" + "="*60)
print("SmolLM MHA within-family summary:")
print(f"  135M: gap_mean={r135['gap_mean_10text']:.4f}, gap_std={r135['gap_std_10text']:.4f}, max_error={r135['max_error_1text']:.3f}")
print(f"  360M: gap_mean={r360['gap_mean_10text']:.4f}, gap_std={r360['gap_std_10text']:.4f}, max_error={r360['max_error_1text']:.3f}")
print(f"  1.7B: gap_mean=0.4200 (est), gap_std=0.063 (from calibration_sensitivity_1b7.json), max_error=0.015")
